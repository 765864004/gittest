...
function initUI(_box){
	...
	if ($.fn.uploadify) {
		$(":file[uploadify]", $p).each(function(){
			var $this = $(this);
			var options = {
				uploader: '/Teach/Backend/Public/plugin/uploadify/uploadify.swf',
				script: $this.attr("script"),
				cancelImg:'/Teach/Backend/Public/plugin/uploadify/cancel.png',
				fileDesc: $this.attr("fileDesc") || "只允许上传mp4类型的视频",
				fileExt : $this.attr("fileExt") || "*.mp4",
    ...
    //以下是我写的插件
	if($.fn.Gauge)$("table[class=GaugeTable]",$p).Gauge();
}
