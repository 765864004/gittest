...
class Page {
	...
	public function show() {
		...
		if ($upRow > 0) {
			$upPage = "<a url='" . $url . "&" . $p . "=$upRow'>" . $this->config['prev'] . "</a>";
		} else {
			$upPage = "";
		}

		if ($downRow <= $this->totalPages) {
			$downPage = "<a url='" . $url . "&" . $p . "=$downRow'>" . $this->config['next'] . "</a>";
		} else {
			$downPage = "";
		}
		// << < > >>
		if ($nowCoolPage == 1) {
			...
			$prePage = "<a url='" . $url . "&" . $p . "=$preRow' >上" . $this->rollPage . "页</a>";
			$theFirst = "<a url='" . $url . "&" . $p . "=1' >" . $this->config['first'] . "</a>";
		}
		if ($nowCoolPage == $this->coolPages) {
			...
			$nextPage = "<a url='" . $url . "&" . $p . "=$nextRow' >下" . $this->rollPage . "页</a>";
			$theEnd = "<a url='" . $url . "&" . $p . "=$theEndRow' >" . $this->config['last'] . "</a>";
		}
		// 1 2 3 4 5
		$linkPage = "";
		for ($i = 1; $i <= $this->rollPage; $i++) {
			...
			if ($page != $this->nowPage) {
				if ($page <= $this->totalPages) {
					$linkPage .= "&nbsp;<a url='" . $url . "&" . $p . "=$page'>&nbsp;" . $page . "&nbsp;</a>";
				} else {
					break;
				}
			} else {
